...
(function() {
  ...
  provide_achra = function() {
    ...
    declare('achra_aform_entry', function(x) {
      return this.isa_object(x);
    });
    //-----------------------------------------------------------------------------------------------------------
    this._colorize_groups = function(groups) {
      var achrs, left, move, right;
      ({left, achrs, right, move} = groups);
      if (move === 'open') {
        return [CND.green(left), CND.yellow('<'), CND.white(achrs), CND.orange(right)].join('');
      }
      return [CND.green(left), CND.white(achrs), CND.yellow('>'), CND.orange(right)].join('');
    };
    //-----------------------------------------------------------------------------------------------------------
    this.achr_pattern = /(?:)/u;
    this.achrs = new Set();
