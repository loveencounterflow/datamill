(function() {
  'use strict';
  var $, GUY, HTMLISH, Pipeline, TIMETUNNEL, _HTMLISH, alert, debug, echo, freeze, help, info, inspect, lets, log, new_markdown_parser, plain, praise, rpr, thaw, urge, warn, whisper, xncr;
...
/* TAINT makes `start`, `stop` invalid (but are thery still needed?) */
      ...
      parse(text, tag_catalog = null) {
        ...
        //-------------------------------------------------------------------------------------------------------
        p.push([text]);
        p.push(this.$tunnel(tunnel_wrap));
        // p.push @$transpile_markdownish()
        p.push(this.$parse_htmlish());
        p.push(function(text) {
          return info('^394^', rpr(text));
        });
        p.push(this.$add_location());
        if (tag_catalog != null) {
          p.push(this.$set_syntax_on_otag(tag_catalog));
        }
        if (tag_catalog != null) {
          p.push(this.$convert_nonhtml_syntax());
        }
        if (tag_catalog != null) {
          p.push(this.$set_syntax_on_other_tokens(tag_catalog));
        }
        p.push(this.$parse_ncrs());
        p.push(this.$complain_about_bareachrs());
        p.push(this.$reveal_tunneled_text(tunnel_wrap));
        p.push(this.$remove_backslashes());
        p.push(this.$treat_xws_in_opening_tags());
        p.push(this.$treat_xws_in_closing_tags());
        p.push(this.$validate_paired_tags());
        p.push(this.$relabel_rawtexts());
