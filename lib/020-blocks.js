(function() {
  'use strict';
  var $, $async, $watch, CND, DATOM, DM, H, SPX, VNR, assign, badge, debug, declare, echo, first, freeze, help, info, is_stamped, isa, jr, last, new_datom, rpr, select, size_of, stamp, thaw, type_of, types, urge, validate, warn, whisper, xr;
    ...
    return $((d, send) => {
      ...
      if ((match = d.text.match(pattern)) != null) {
        ...
        if (within_codeblock) {
          ...
          //.....................................................................................................
          /* TAINT why not use `fresh_datom()`? */          send(stamp(d));
          send(DATOM.set(d, {
            $vnr,
            $key: '>codeblock',
            ref: 'blk/cdb3'
        ...
        if (/* TAINT why not use `fresh_datom()`? */within_codeblock) {
          d = DATOM.set(d, {
            $key: '^literal',
            ref: 'blk/cdb4'
          });
          /* TAINT why not use `fresh_datom()`? */          send(d);
        } else {
          send(d);
        }
      }
