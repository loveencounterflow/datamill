(function() {
  ...
  Datamill_server_base = class Datamill_server_base {
    ...
    with_html_stream(ctx, f) {
      var line, ref, ref1, stream;
      ctx.response.type = 'html';
      ctx.body = stream = new Stream();
      ref = this.doc.db.first_values(SQL`select doc_line_txt from doc_lines`);
      /* TAINT use API to stream `prologue` part of layout */
      for (line of ref) {
        stream.write(line);
      }
      f.call(this, stream);
      ref1 = this.doc.db.first_values(SQL`select doc_line_txt from doc_lines`);
...
<ul>
  ...
  order by doc_line_nr;`);
    ...
    _r_files(ctx) {
      ...
      /* TAINT use API */
      /* TAINT respect custom table prefix */
      this.with_html_stream(ctx, function({push, write}) {
        var file, href, ref;
