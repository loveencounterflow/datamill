(function() {
  ...
  this.Datamill_server = class Datamill_server {
    ...
    constructor(cfg) {
      ...
      //---------------------------------------------------------------------------------------------------------
      this._r_relations = this._r_relations.bind(this);
      // #---------------------------------------------------------------------------------------------------------
      // _r_trends: ( ctx ) =>
    ...
    _create_app() {
      ...
      */
      ...
      this.router.get('relations', '/relations', this._r_relations);
...
<ul>
  <li><a href=relations>Relations (Tables &amp; Views)</a></li>
    ...
    _r_relations(ctx) {
      ...
      ref = this.db(SQL`select * from sqlite_schema order by name;`);
      for (row of ref) {
        content.push(HDML.open('tr'));
        content.push(HDML.pair('td', HDML.text(row.type)));
        content.push(HDML.pair('td', HDML.text(row.name)));
        content.push(HDML.pair('td', HDML.pair('pre', HDML.text((ref1 = row.sql) != null ? ref1 : './.'))));
      }
      content.push(HDML.close('table'));
      ctx.body = content.join('\n');
      rows = this.db.all_rows(SQL`select * from sqlite_schema order by name;`);
      ctx.body += '<hr>';
      ctx.body += tabulate({rows, ...(this._get_table_cfg('relations'))});
      return null;
    }
    ...
    _set_table_cfgs() {
      ...
      this.table_cfgs['trends'] = {
        fields: {
          ...
          ts: {
            inner_html: (d) => {
              return this.db.dt_format(d.value, 'YYYY-MM-DD HH:mm UTC');
            }
          },
          //...................................................................................................
          raw_trend: {
