(function() {
  ...
  primary key ( doc_file_id ) );`);
        //.......................................................................................................
        this.db(SQL`create view ${prefix}raw_lines as select
  ...
  check ( doc_loc_kind in ( 'start', 'stop' ) ) );`);
        ...
        this._insert_loc_alt_ps = this.db.alt.prepare_insert({
          into: `${prefix}locs`
        });
        return null;
      }
      ...
      walk_raw_lines(cfg, ...P) {
        ...
        if (cfg.length === 0) {
          return this.db(this._raw_lines_ps);
        }
        sql = [];
        ({L} = this.db.sql);
/* TAINT can probably may simpler by using a join */
        for (idx = i = 0, len = cfg.length; i < len; idx = ++i) {
          doc_file_id = cfg[idx];
          sql.push(SQL`select ${L(idx + 1)} as doc_file_nr, R.doc_file_id, R.doc_line_nr, R.doc_par_nr, R.doc_line_txt ` + SQL`from ${this.cfg.prefix}raw_lines as R where R.doc_file_id = ${L(doc_file_id)}\n`);
        }
      ...
      //---------------------------------------------------------------------------------------------------------
      add_file(cfg) {
        var R, doc_file_abspath, doc_file_hash, doc_file_id, doc_file_path;
        ...
        R = this.db.first_row(this._insert_file_ps, {doc_file_id, doc_file_path, doc_file_hash});
        /* TAINT only when licensed by extension `*.dm.*` or settings */
        this._add_locs_for_file(R);
        return R;
      }
      ...
      _add_locs_for_file(file) {
        this.db.alt(() => {
          ...
          for (loc of ref) {
            this.db.alt(this._insert_loc_alt_ps, loc);
          }
          return null;
        });
        return null;
      ...
      * _walk_locs_of_file(file) {
        var doc_file_id, doc_line_nr, doc_loc_kind, doc_loc_mark, doc_loc_name, doc_loc_start, doc_loc_stop, left_slash, length, line, match, ref, ref1, right_slash, text;
        ({doc_file_id} = file);
        ref = this.walk_raw_lines([doc_file_id]);
        for (line of ref) {
          ({doc_line_nr} = line);
          ref1 = line.doc_line_txt.matchAll(this.cfg.loc_marker_re);
          for (match of ref1) {
