    ...
    and doc_line_nr = (
      ...
      * walk_raw_lines(...region_ids) {
      ...
      walk_loc_lines(cfg, ...P) {
        ...
        for (idx = i = 0, len = cfg.length; i < len; idx = ++i) {
          doc_src_id = cfg[idx];
          sql.push(SQL`select ${L(idx + 1)} as doc_src_nr, * ` + SQL`from ${this.cfg.prefix}xxx_lines as R where R.doc_src_id = ${L(doc_src_id)}\n`);
        }
      ...
      * _walk_locs_of_file(file) {
      ...
      _file_from_abspath(doc_src_abspath) {
        return this.db.first_row(SQL`select * from ${this.cfg.prefix}sources where doc_src_abspath = $doc_src_abspath`, {doc_src_abspath});
      }
