  ...
  where true
    ...
    and doc_line_nr = (
      ...
      * walk_raw_lines(...region_ids) {
        ...
        var doc_file_id, doc_file_nr, doc_loc_name, i, idx, len, line, ref, region_id;
        region_ids = region_ids.flat(2e308);
        for (idx = i = 0, len = region_ids.length; i < len; idx = ++i) {
          region_id = region_ids[idx];
          ({doc_file_id, doc_loc_name} = this._split_region_id(region_id));
          doc_file_nr = idx + 1;
          ref = this.db(this._raw_lines_ps, {doc_file_nr, doc_file_id});
          for (line of ref) {
            yield line;
          }
        }
