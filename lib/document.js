(function() {
  ...
  check ( doc_loc_kind in ( 'start', 'stop' ) ) );`);
        ...
        this._delete_file_ps = this.db.prepare(SQL`delete from ${prefix}files where doc_file_id = $doc_file_id;`);
  ...
  select * from ${prefix}live_raw_lines
    where doc_file_id = $doc_file_id;`);
        this._raw_lines_ps = this.db.prepare(SQL`select * from ${prefix}raw_lines`);
  ...
  where true
    ...
    and doc_line_nr = (
      ...
      walk_raw_lines(cfg, ...P) {
        var doc_file_id, i, idx, len;
        if (P.length !== 0) {
          return this.walk_raw_lines([...arguments]);
        }
        if (cfg == null) {
          cfg = [];
        }
        cfg = this.types.create.walk_raw_lines_cfg(cfg);
        if (cfg.length === 0) {
          return [];
        }
        sql = [];
...
/* TAINT can probably may simpler by using a join */
        for (idx = i = 0, len = cfg.length; i < len; idx = ++i) {
          doc_file_id = cfg[idx];
          sql.push(SQL`select ${L(idx + 1)} as doc_file_nr, * ` + SQL`from ${this.cfg.prefix}raw_lines as R where R.doc_file_id = ${L(doc_file_id)}\n`);
        }
        return this.db(sql.join('union all\n'));
      }

      //---------------------------------------------------------------------------------------------------------
      walk_xxx_lines(cfg, ...P) {
        var doc_file_id, i, idx, len;
        if (P.length !== 0) {
...
/* TAINT can probably may simpler by using a join */
        for (idx = i = 0, len = cfg.length; i < len; idx = ++i) {
          doc_file_id = cfg[idx];
          sql.push(SQL`select ${L(idx + 1)} as doc_file_nr, * ` + SQL`from ${this.cfg.prefix}xxx_lines as R where R.doc_file_id = ${L(doc_file_id)}\n`);
        }
        return this.db(sql.join('union all\n'));
      }
      ...
      //---------------------------------------------------------------------------------------------------------
      add_file(cfg) {
        var doc_file_abspath, doc_file_hash, doc_file_id, doc_file_path, file;
        ...
        file = this.db.first_row(this._insert_file_ps, {doc_file_id, doc_file_path, doc_file_hash});
        this.db.alt(this._insert_lines_2ps, {doc_file_id});
      ...
      * _walk_locs_of_file(file) {
        var doc_file_id, doc_line_nr, doc_loc_kind, doc_loc_mark, doc_loc_name, doc_loc_start, doc_loc_stop, left_slash, length, line, match, ref, ref1, right_slash, stop, text;
        ({doc_file_id} = file);
        //.......................................................................................................
        ({doc_line_nr, stop} = this._get_last_position_in_file(doc_file_id));
        yield ({
          doc_file_id,
          doc_line_nr: 1,
          doc_loc_name: '*',
        ...
        ref = this.walk_raw_lines([doc_file_id]);
        //.......................................................................................................
        for (line of ref) {
          ({doc_line_nr} = line);
          ref1 = line.doc_line_txt.matchAll(this.cfg._loc_marker_re);
          for (match of ref1) {
            ({left_slash, doc_loc_name, right_slash} = match.groups);
            [text] = match;
            length = text.length;
            ({
              index: doc_loc_start
            ...
            if ((left_slash === '') && (right_slash === '')) {
              ...
              doc_loc_kind = 'start';
              doc_loc_mark = doc_loc_stop;
              yield ({doc_file_id, doc_line_nr, doc_loc_name, doc_loc_kind, doc_loc_start, doc_loc_stop, doc_loc_mark});
              doc_loc_kind = 'stop';
            } else {
      ...
      _get_last_position_in_file(doc_file_id) {
        ...
        ({doc_line_nr, doc_line_txt} = this.db.first_row(this._last_line_ps, {doc_file_id}));
        return {
          doc_line_nr,
      ...
      //---------------------------------------------------------------------------------------------------------
      _delete_file(doc_file_id) {
        return this.db(this._delete_file_ps, {doc_file_id});
      }
