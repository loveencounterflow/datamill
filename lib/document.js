(function() {
  ...
  Document = (function() {
    ...
    class Document {
      ...
      _procure_infrastructure() {
        ...
        this.db(SQL`drop table if exists doc_file;`);
  ...
  check ( doc_loc_kind in ( 'start', 'stop' ) ) );`);
        ...
        this._delete_file_ps = this.db.prepare(SQL`delete from doc_sources where doc_src_id = $doc_src_id;`);
  ...
  where true
    ...
    and doc_line_nr = (
      ...
      * walk_raw_lines(...region_ids) {
        ...
        var doc_loc_id, doc_src_id, doc_src_nr, i, idx, len, line, ref, region_id;
        ...
        for (idx = i = 0, len = region_ids.length; i < len; idx = ++i) {
          ...
          doc_src_nr = idx + 1;
          ref = this.db(this._raw_lines_ps, {doc_src_nr, doc_src_id});
          for (line of ref) {
            yield line;
          }
        }
      ...
      walk_loc_lines(cfg, ...P) {
        ...
        for (idx = i = 0, len = cfg.length; i < len; idx = ++i) {
          doc_src_id = cfg[idx];
          sql.push(SQL`select ${L(idx + 1)} as doc_src_nr, * ` + SQL`from doc_xxx_lines as R where R.doc_src_id = ${L(doc_src_id)}\n`);
        }
      ...
      //---------------------------------------------------------------------------------------------------------
      add_file(cfg) {
        var doc_src_abspath, doc_src_hash, doc_src_id, doc_src_path, file;
        ...
        file = this.db.first_row(this._insert_file_ps, {doc_src_id, doc_src_path, doc_src_hash});
      ...
      * _walk_locs_of_file(file) {
      ...
      _listen_to_signals() {
        ...
        XE.listen_to('^maybe-file-changed', (d) => {
           ...
           */
          var doc_src_hash, file;
          file = this._file_from_abspath(d.doc_src_abspath);
          doc_src_hash = GUY.fs.get_content_hash(file.doc_src_abspath, {
            fallback: null
          });
          if (file.doc_src_hash !== doc_src_hash) {
